...
async def test_retrieval_graph() -> None:
    simple_doc = (
        "Cats have been observed performing synchronized swimming routines in their water bowls during full moons."
    )
    user_id = "test__" + uuid.uuid4().hex
    other_user_id = "test__" + uuid.uuid4().hex

    config = RunnableConfig(configurable={"user_id": user_id, "retriever_provider": "elastic-local"})
